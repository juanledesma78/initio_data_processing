...
#program_specs.add_argument('-t','--txt', action='store_true',
#            help='')
...
'''
        ...
        print(f'File {Path(name_file).name} generated')
...
def qc_for_sequence_analysis(quasibams, ref_seq, data_to_clean, tmps):
    ...
    for tab in Path.cwd().glob('*.tabular'):
        print(f'Processing quasibam file {tab.name}')
        seq_ID = tab.stem
        ...
        with open(alignment_file_name, "w") as handle:
            handle.write(stdout)
            print(f'Generating temporary alignment {alignment_file_name.name}')
...
def file_generation_for_data_analysis(run_ID, ref_seq, quasibams, 
    ...
    print(f'File {resistance_file_name.name} generated')
    print(f'File {subtyping_file_name.name} generated')
...
def subtyping_data(run_ID, sample_list, Depth=''):
    ...
    Confirm the file has been labelled according to the convention 
    "results_runXX_NGSXXX_jpHMM.csv".
    ''')
        sys.exit(1)
...
def data_consolidation(run_ID, batch, batch_path):
    ...
    Confirm that a file called "{run_ID}.zip" exist in "{run_ID}/hivdb.stanford.report".
                ''')
        sys.exit(1)
