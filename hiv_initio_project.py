...
program_specs.add_argument('-i', '--input', required= True, 
            help='Path to run containing the data to be analysed.\
                This is REQUIRED for using all the other arguments.')

program_specs.add_argument('-p','--postgenomancer',  #action='store_true',
            nargs='?', const= 'Mm100',
            help='It takes as an argument the location of the run, decompresses\
                the files "*_quasibams.zip","*_post-run.zip" and "*_typonomer.zip",\
                run QuasiBAM script to generate sequences of Majority and Minority\
                variants (frequency of 20PC and 2PC) at depth of 100, creates\
                an XLSX file to summarise the results generated by the pipeline \
                Genomancer and collates the coverage depth of all the sequences in a\
                dataframe.  I also extracts each fasta sequence from the file\
                "*_genomes.fas" into a new directory called "FASTAs".\
                [ADDITIONAL VALUE]: The argument -p/--postgenomancer can take a\
                value to produce additional sequences at depth of 30 reads for\
                Majority variants (20PC). If needed enter the value "m30" after\
                the argument.') 
                # symbol % gives an error if included in help 

program_specs.add_argument('-qc', '--quality', action='store_true',
            help='It takes as an argument the location of the run, aligns\
                a reference sequence and the nucloetide sequences from the\
                same sample and save the alignmets in a directory called\
                "data_to_clean", containing the sequences to analyse.\
                A TXT report with amino acids sequences, potential frame\
                shifts and stop codons (if present) is created in "quasibams".\
                An XLSX file containing the Quasibam file with an extra\
                column with the approximate positions according to the fasta\
                20 PC.')

program_specs.add_argument('-c','--confirmation', action='store_true',
            help='It takes as an argument the location of the NGS run and\
                creates a second TXT report in "quasibams" to confirm the\
                sequences are in frame.')

program_specs.add_argument('-f' ,'--files', action='store_true',
            help='It takes as an argument the location of the NGS run and\
                stores the files to use for subtyping and antiviral resistance\
                genotyping in a directory called "fastas" in the run directory.\
                The folders "hivdb.stanford.report" and "subtyping", used to save\
                the results after using Stanford algorithm and subtyping tools,\
                are created as well.')

program_specs.add_argument('-d','--data', action='store_true',
            help='It takes as an argument the location of the NGS run and\
                consolidates all the sample information and the subtyping\
                reports and resistance genotyping gerenated using sequences\
                at depth of 100 reads and frequency of 20PC and 2PC (Majority\
                and Minority Variants, respectively) in an XLSX file in the\
                main directory of the INITIO batch.')
...
'''
        ...
        for gene, pos in gene_Dictionary.items(): 
            ...
            ''' This is the tool to check frame-shifts'''
            ...
            for n in range(len(NT_query_seqs)):
                ...
                if '*' in frames[0][:-1]:
                    frame_analysis.append('STOP CODON(s) detected across the sequence.')
...
def sample_info(run_ID):
    ...
    RUNXXX__NGS_QC.csv is created from the worksheet tab "Enter QC data" of
    "VW1922.11_Calculator_For_Pooling_Libraries_*.xlsm", which has had different
    formats affecting the column headers. If the worksheet is changed again and 
    affects the headers this should be considered to avoid errors in the csv
